...
| `ANTHROPIC_API_KEY`                                   | Anthropic API key for Claude Haiku vision — used by `/api/admin/certifications/extract` to auto-fill form fields from certificate images. Optional; feature degrades gracefully if unset. Set in Amplify Console + `amplify.yml`. |
