...
- **Signature accent (`--accent-signature`):** The brand orange token (`oklch(0.75 0.15 55)` light / `oklch(0.65 0.1 55)` dark, defined in `globals.css`). Used for active state indicators across the public site: nav link underlines, tab active bar, mobile nav left-border. Reference via `var(--accent-signature)` in inline styles or as an arbitrary Tailwind value (e.g. `border-l-[var(--accent-signature)]`). Do not hardcode the orange color.
