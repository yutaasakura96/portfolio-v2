...
Prisma 7.4.1 + Neon Postgres via `@prisma/adapter-neon` (`PrismaNeon` — WebSocket adapter). Generated client output is `generated/prisma/`. Single schema file at [prisma/schema.prisma](prisma/schema.prisma). Both `@neondatabase/serverless` and `@prisma/adapter-neon` must stay in `serverExternalPackages` in `next.config.ts` to avoid Lambda bundling issues.
