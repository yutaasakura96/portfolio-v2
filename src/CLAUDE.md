...
- **Hooks: `use-kebab-case.ts`** in [src/hooks/](src/hooks/) (matching existing `use-auth.ts`, `use-dashboard-stats.ts`, `use-dashboard-external.ts`, `use-messages.ts`, `use-dnd-reorder.ts`, `use-reveal.ts`, `use-settings.ts`). Generic reusable hooks belong here — e.g. `use-dnd-reorder.ts` encapsulates dnd-kit draft state + TanStack Query save mutation for any entity with `displayOrder`.
