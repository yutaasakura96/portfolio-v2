import { prisma, withRetry } from "@/lib/prismaClient";
import type {
  AboutPage,
  AboutPageData,
  AdjacentProjects,
...
export async function getAboutPageIntro(): Promise<AboutPage | null> {
  try {
    return await withRetry(() => prisma.aboutPage.findUnique({ where: { id: "default" } }));
  } catch (error) {
...
export async function getHero(): Promise<Hero | null> {
  try {
    return await withRetry(() => prisma.hero.findFirst());
  } catch (error) {
...
export async function getAdjacentProjects(currentOrder: number): Promise<AdjacentProjects> {
  try {
    const [prev, next] = await withRetry(() =>
      Promise.all([
...
export async function getSiteSettings(): Promise<SiteSettings | null> {
  try {
    return await withRetry(() => prisma.siteSettings.findFirst());
  } catch (error) {
