...
- **`GET /api/admin/dashboard-stats`** — admin-only aggregate endpoint. Returns counts (projects, posts, messages, skills), published/draft breakdowns, certification expiry data, recent unread messages, merged recent activity (projects + posts sorted by `updatedAt`), a content completeness checklist, and `translationStats` (JA translation coverage across 7 entity types). Uses `requireAuthOrApiKey`, uses `groupBy` to consolidate counts (projects by status, posts by status, messages by read/archived). Response shape: `{ data: DashboardStats }` (single resource envelope).
- **`GET /api/admin/dashboard-external`** — admin-only endpoint that fetches live status from external services in parallel: Sentry REST API, AWS Amplify SDK (dynamic import), production health ping, and GA config check. Each source degrades gracefully (returns `{ configured: false }` or `null`) when env vars are missing. Uses `requireAuthOrApiKey`, `AbortSignal.timeout()` for external HTTP calls. Response shape: `{ data: ExternalStats }` (single resource envelope).
