...
const config = entityConfigs.experience;
...
export const GET = withErrorHandler(async (request: NextRequest) => {
  ...
  const rows = await prisma.experience.findMany({ orderBy: config.orderBy });
  const stripped = stripInternalFieldsFromArray(rows as unknown as Record<string, unknown>[]);
  const filename = getExportFilename("experience", format);
