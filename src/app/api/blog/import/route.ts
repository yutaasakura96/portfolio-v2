...
export const POST = withErrorHandler(async (request: NextRequest) => {
  ...
  await prisma.$transaction(async (tx) => {
    for (const item of items) {
      ...
      const existing = where ? await tx.blogPost.findFirst({ where }) : null;

      if (existing) {
        if (mode === "create") {
          skipped++;
        } else {
          await tx.blogPost.update({ where: { id: existing.id }, data: item as never });
          updated++;
        }
      } else {
        await tx.blogPost.create({ data: item as never });
        created++;
      }
    }
  });
