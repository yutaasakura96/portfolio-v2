...
export const POST = withErrorHandler(async (request: NextRequest) => {
  ...
  await prisma.$transaction(async (tx) => {
    for (const item of items) {
      ...
      const existing = where ? await tx.project.findFirst({ where }) : null;

      if (existing) {
        if (mode === "create") {
          skipped++;
        } else {
          await tx.project.update({ where: { id: existing.id }, data: item as never });
          updated++;
        }
      } else {
        await tx.project.create({ data: item as never });
        created++;
      }
    }
  });
