...
export const GET = withErrorHandler(async (request: NextRequest) => {
  ...
  if (format !== "json") {
    throw new ApiError("About export only supports JSON format.", 400, ErrorCodes.VALIDATION_ERROR);
  }

  const about = await prisma.aboutPage.findFirst();
  if (!about) {
    throw new ApiError("No about page data found.", 404, ErrorCodes.NOT_FOUND);
  }

  const stripped = stripInternalFields(about as unknown as Record<string, unknown>);
  const filename = getExportFilename("about", "json");
